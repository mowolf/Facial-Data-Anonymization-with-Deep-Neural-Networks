...
def resnet50_v1s(pretrained=False, root='~/.torch/models', **kwargs):
    # {'dilated': True, 'local_rank': 0, 'norm_layer': <class 'torch.nn.modules.batchnorm.BatchNorm2d'>}
    # model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3], deep_stem=True, **kwargs )
    model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3], deep_stem=True, dilated=kwargs["dilated"], norm_layer=kwargs["norm_layer"] )
    if pretrained:
